/**
 * UI language.
 *
 * Three languages because the audience genuinely splits three ways: Armenian
 * accountants working in Armenian, the large Russian-working professional
 * community, and English for anyone advising from outside. This is the
 * INTERFACE language only — answers still mirror the language of the question,
 * and Armenian legal quotations stay Armenian in every case, because they are
 * the authoritative text and translating them would defeat the point.
 *
 * A flat dictionary, no i18n library: the string count is small, and a
 * dependency would buy pluralisation rules and lazy loading we do not need.
 */
export type Lang = 'hy' | 'ru' | 'en';

export const LANGS: { code: Lang; label: string }[] = [
  { code: 'hy', label: 'ՀԱՅ' },
  { code: 'ru', label: 'РУС' },
  { code: 'en', label: 'ENG' },
];

type Dict = Record<string, string>;

const hy: Dict = {
  'nav.consultations': 'Խորհրդատվություններ',
  'nav.newCase': '+ Նոր խորհրդատվություն',
  'nav.noCases': 'Պահպանված երկխոսություններ դեռ չկան։',
  'nav.search': 'Որոնել զրույցներում',
  'nav.noMatches': 'Համընկնումներ չեն գտնվել։',
  'nav.more': 'Գործողություններ',
  'nav.pin': 'Ամրացնել',
  'nav.unpin': 'Հանել ամրացումը',
  'nav.pinned': 'Ամրացված',
  'nav.showAllPinned': 'Ցույց տալ բոլոր ամրացվածները',
  'nav.showLess': 'Ավելի քիչ',
  'nav.recent': 'Վերջին',
  'nav.openRail': 'Բացել կողագոտին',
  'nav.rename': 'Վերանվանել',
  'nav.renameTitle': 'Վերանվանել զրույցը',
  'nav.renameHint': 'Տվեք անուն, որով հեշտ կգտնեք այս զրույցը։ Դատարկ թողեք՝ առաջին հարցին վերադառնալու համար։',
  'nav.delete': 'Ջնջել',
  'nav.deleteAsk': 'Ջնջե՞լ ընդմիշտ',
  'nav.deleteYes': 'Ջնջել',
  'nav.deleteTitle': 'Ջնջե՞լ զրույցը',
  'nav.deleteBody': 'Զրույցը կջնջվի ընդմիշտ, և Դուք այլևս մուտք չեք ունենա դրան։ Այս գործողությունը հնարավոր չէ հետարկել։',
  'nav.cancel': 'Չեղարկել',
  'nav.save': 'Պահպանել',
  'account.profile': 'Պրոֆիլ',
  'account.profileTitle': 'Փոխել անունը',
  'account.profileHint': 'Այս անունով ձեզ տեսնում են ձեր աշխատանքային տարածքի անդամները։',
  'account.workspace': 'Աշխատատարածք',
  'account.upgrade': 'Ընդլայնել սակագինը',
  'account.upgradeSoon': 'Վճարումները դեռ միացված չեն։',
  'account.questions': 'Հարցեր այս շաբաթ',
  'ws.members': 'Անդամներ',
  'ws.invitees': 'Հրավիրվածներ',
  'ws.usage': 'Օգտագործում',
  'ws.newMember': 'Նոր անդամ',
  'ws.send': 'Հրավիրել',
  'ws.remove': 'Հեռացնել',
  'ws.removeYes': 'Հաստատել',
  'ws.removeMemberTitle': 'Հեռացնե՞լ անդամին',
  'ws.removeMemberBody': 'Այս անձը կհեռացվի Ձեր աշխատանքային տարածքից և այլևս չի օգտագործի թիմի շաբաթական հարցերը։ Նրա հաշիվը և զրույցները կպահպանվեն։',
  'ws.removeOwnerNote': 'Սա աշխատանքային տարածքի ստեղծողն է։ Հեռացնելուց հետո սեփականատերը կդառնաք Դուք։',
  'ws.removeInviteTitle': 'Հեռացնե՞լ հրավերը',
  'ws.removeInviteBody': 'Հրավերի հղումն այլևս չի գործի։ Կարող եք ցանկացած ժամանակ նորից հրավիրել։',
  'ws.promote': 'Դարձնել ադմին',
  'ws.demote': 'Հանել ադմինը',
  'ws.roleLabel': 'Իրավունքները',
  'ws.nameRequired': 'Գրեք անունը՝ հրավիրվածն ինքը չի կարող այն լրացնել։',
  'ws.roleRequired': 'Ընտրեք՝ ադմին, թե օգտվող։',
  'ws.roleAdmin': 'Ադմին',
  'ws.roleMember': 'Օգտվող',
  'ws.pending': 'Սպասում է',
  'ws.you': 'Դուք',
  'ws.alreadyHere': 'Այս հասցեն արդեն ձեր թիմում է կամ հրավիրված է։',
  'ws.full': 'Թիմը լրացված է։',
  'ws.failed': 'Չհաջողվեց։ Փորձեք կրկին։',
  'ws.totalThisMonth': 'Ընդամենը այս շաբաթ',
  'ws.inviteNote': 'Հրավերն ուղարկվում է նշված հասցեին։ Գործում է միայն այդ հասցեով գրանցվելիս։',

  'mode.chat': 'Երկխոսություն',
  'mode.ask': 'Մեկանգամյա',
  'mode.search': 'Որոնում',
  'mode.chatHint': 'Երկխոսություն հիշողությամբ — ճշտեք և խնդրեք եզրակացություն ամբողջի վերաբերյալ։',
  'mode.askHint': 'Մեկ հարց — մեկ պատասխան, առանց հիշողության։',
  'mode.searchHint': 'Միայն գտնված հատվածները, առանց մոդելի։',

  'corpus.acts': 'ակտ',
  'corpus.chunks': 'հատված',
  'corpus.synced': 'Ստուգված ARLIS-ի հետ',
  'corpus.disclaimer': 'Սա իրավաբանական խորհրդատվություն չէ — ստուգեք ամբողջական տեքստը',

  'turn.question': 'Հարց',
  'turn.searchedFor': 'որոնվել է՝',

  'stage.understanding': 'Վերլուծում եմ հարցը…',
  'stage.searching': 'Փնտրում եմ Հարկային օրենսգրքում…',
  'stage.reading': 'Կարդում եմ գտնված հոդվածները…',
  'stage.writing': 'Ձևակերպում եմ պատասխանը…',

  'coverage.partial':
    'Գտնված հոդվածները հարցը լուսաբանում են մասամբ — ստուգեք, արդյոք ճշտող հարցը չի փոխում եզրակացությունը։',
  'coverage.none':
    'Գտնված հոդվածներում այս հարցին ուղղակի պատասխանող նորմ չկա։ Ստորև՝ միայն հարակից դրույթներ։',


  'composer.first': 'Նկարագրեք իրավիճակը կամ տվեք հարց…',
  'composer.next': 'Ճշտող հարց կամ «եզրակացրու ամբողջ քննարկվածից»',
  'composer.send': 'Ուղարկել',

  'norm.empty':
    'Տվեք հարց — այստեղ կհայտնվի այն հոդվածը, որի վրա հենվում է պատասխանը՝ ընդգծված մեջբերումով և խմբագրության ամսաթվով։',
  'norm.inForce': 'Գործում է',
  'norm.revised': 'Խմբ.',
  'norm.adopted': 'Ընդունված',
  'norm.revisedFrom': 'Խմբ. թիվ',
  'norm.checked': 'Ստուգված',
  'norm.copyQuote': 'Պատճենել մեջբերումը',
  'norm.copyArticle': 'Պատճենել հոդվածը',
  'norm.copied': 'Պատճենվեց',
  'norm.refersTo': 'Հղվում է',

  'login.note':
    'Մուտքը՝ ընդհանուր գաղտնաբառով։ Յուրաքանչյուր պատասխան ծախսում է վճարովի API-հարցումներ, ուստի գործիքը փակ է։',
  'login.password': 'Գաղտնաբառ',
  'login.enter': 'Մուտք',
  'login.wrong': 'Սխալ գաղտնաբառ։',
  'auth.signIn': 'Մուտք',
  'auth.register': 'Գրանցում',
  'auth.haveAccount': 'Արդեն ունե՞ք հաշիվ։',
  'auth.noAccount': 'Դեռ չունե՞ք հաշիվ։',
  'auth.signInNote':
    'Մուտք գործեք՝ Ձեր զրույցները տեսնելու համար։ Յուրաքանչյուրը տեսնում է միայն իրենը։',
  'auth.registerNote': 'Ստեղծեք հաշիվ։ Անվճար փաթեթը ներառում է շաբաթական 5 հարց։',
  'auth.email': 'Էլ. փոստ',
  'auth.name': 'Անուն',
  'auth.fullName': 'Անուն Ազգանուն',
  'auth.companyName': 'Ընկերության անվանում',
  'auth.companySize': 'Ընկերության չափը',
  'auth.next': 'Շարունակել',
  'auth.inviteOffer':
    'Հրավիրեք գործընկերներին և ստացեք +{n} հարց։ Յուրաքանչյուր գրանցվողի համար՝ ևս +{m}։',
  'auth.inviteNote':
    'Ըստ ցանկության։ +{m} հարցը ավելանում է այն պահին, երբ գործընկերը գրանցվում է։ Հասցեները չենք փոխանցում երրորդ անձանց։',
  'auth.skip': 'Բաց թողնել',
  'auth.inviteAnother': 'Ավելացնել ևս մեկը',
  'auth.whyWeAsk':
    'Այս տվյալները օգտագործում ենք միայն Ձեզ համապատասխան փաթեթը առաջարկելու և պատասխանները Ձեր գործունեությանը հարմարեցնելու համար։ Երրորդ անձանց չենք փոխանցում։',
  'auth.createAccount': 'Ստեղծել հաշիվ',
  'auth.or': 'կամ',
  'reset.forgot': 'Մոռացե՞լ եք գաղտնաբառը',
  'reset.send': 'Ուղարկել վերականգնման հղումը',
  'reset.maybeSent': 'Եթե այս հասցեով հաշիվ կա, հղումն ուղարկվեց։ Գործում է 1 ժամ։',
  'reset.chooseNew': 'Ընտրեք նոր գաղտնաբառ։',
  'reset.save': 'Պահպանել գաղտնաբառը',
  'reset.invalid': 'Հղումն անվավեր է։',
  'reset.expired': 'Հղումը ժամկետանց է։ Պահանջեք նորը։',
  'reset.used': 'Հղումն արդեն օգտագործված է։ Պահանջեք նորը։',
  'invite.loading': 'Բացվում է հրավերը…',
  'invite.gone': 'Հրավերն անվավեր է կամ արդեն ընդունված։',
  'invite.invitedYou': 'հրավիրել է Ձեզ։ Մնում է ընտրել գաղտնաբառ։',
  'invite.accept': 'Ընդունել հրավերը',
  'invite.failed': 'Չհաջողվեց։ Փորձեք կրկին։',
  'invite.thenMember': 'Հաստատումից հետո կհայտնվեք թիմի անդամների ցանկում։',
  'auth.oauth.noAccount': 'Հաշիվ չի գտնվել։ Գրանցվեք։',
  'auth.oauth.cancelled': 'Մուտքը չեղարկվեց։',
  'auth.oauth.failed': 'Google-ով մուտքը չհաջողվեց։ Փորձեք կրկին։',
  'auth.oauth.unverified': 'Google հասցեն հաստատված չէ։',
  'auth.google': 'Շարունակել Google-ով',
  'auth.badCredentials': 'Սխալ էլ. փոստ կամ գաղտնաբառ։',
  'auth.verify.title': 'Ստուգեք Ձեր փոստը',
  'auth.verify.body': 'Հաստատման հղումն ուղարկվեց այս հասցեին․',
  'auth.verify.hint': 'Հղումը գործում է 24 ժամ։ Չե՞ք գտնում — նայեք սպամի պանակում։',
  'auth.verify.resend': 'Ուղարկել կրկին',
  'auth.verify.resent': 'Ուղարկվեց',
  'auth.verify.unverified': 'Հասցեն դեռ հաստատված չէ։ Ստուգեք փոստը կամ ուղարկեք հղումը կրկին։',
  'auth.verify.sendFailed': 'Նամակը չհաջողվեց ուղարկել։ Փորձեք կրկին ուղարկել։',
  'auth.verify.working': 'Հաստատվում է…',
  'auth.verify.invalid': 'Հղումն անվավեր է։',
  'auth.verify.expired': 'Հղումը ժամկետանց է։ Ուղարկեք նորը։',
  'auth.verify.alreadyUsed': 'Հասցեն արդեն հաստատված է։ Կարող եք մուտք գործել։',
  'auth.verify.toSignIn': 'Դեպի մուտք',
  'auth.emailTaken': 'Այս էլ. փոստով հաշիվ արդեն կա։',
  'auth.weakPassword': 'Գաղտնաբառը պետք է լինի առնվազն 8 նիշ։',
  'auth.invalidEmail': 'Էլ. փոստի սխալ հասցե։',
  'auth.confirmPassword': 'Կրկնեք գաղտնաբառը',
  'auth.passwordMismatch': 'Գաղտնաբառերը չեն համընկնում։',
  'auth.showPassword': 'Ցույց տալ գաղտնաբառը',
  'auth.hidePassword': 'Թաքցնել գաղտնաբառը',
  'auth.signOut': 'Ելք',
  'auth.quotaOut': 'Այս շաբաթվա սահմանաչափը սպառված է։',
  'share.share': 'Կիսվել',
  'share.title': 'Կիսվել զրույցով',
  'share.copy': 'Պատճենել',
  'share.failed': 'Չհաջողվեց ստանալ հղումը։ Փորձեք կրկին։',
  'share.publicNote': 'Հղումը հանրային է․ այն ունեցող ցանկացած անձ, առանց հաշվի, կարող է կարդալ այս զրույցը։',
  'share.copied': 'Հղումը պատճենվեց',
  'share.stop': 'Դադարեցնել կիսվելը',
  'share.shared': 'Կիսված',
  'share.gone': 'Այս հղումը այլևս գործող չէ։',
  'share.readOnly': 'Կիսված խորհրդատվություն',
  'share.ownQuestion': 'Ունե՞ք Ձեր հարցը։',
  'share.openTool': 'Բացել MatyanAI-ն',
  'preview.lede': 'Տվեք Ձեր իրավական հարցը։',
  'preview.ask': 'Հարցնել',
  'preview.thinking': 'Փնտրում եմ…',
  'preview.rest': 'Պատասխանի մնացած մասը՝ հոդվածներով և բառացի մեջբերումներով',
  'preview.sources': 'հոդված',
  'preview.unlock': 'Գրանցվել և տեսնել ամբողջը',
  'preview.free': 'Անվճար · շաբաթական 5 հարց',
  'preview.failed': 'Չհաջողվեց ստանալ պատասխանը։',
  'preview.resuming': 'Պատրաստում եմ Ձեր ամբողջական պատասխանը…',

  'theme.auto': 'Ավտո',
  'theme.light': 'Լույս',
  'theme.dark': 'Մուգ',

  'error.credit':
    'Anthropic API-ի հաշվեկշիռը սպառվել է — սա ծրագրի սխալ չէ։ Համալրեք հաշիվը Plans & Billing բաժնում; որոնումն աշխատում է առանց դրա։',
  'error.noApi': 'API-ն չի պատասխանում',
  'norm.title': 'Աղբյուրներ',
  'norm.carried': 'Փոխանցված է նախորդ հաղորդագրություններից — կարդացվել է, բայց այս պատասխանում չի մեջբերվել։',
  'norm.openArlis': 'Բացել ARLIS-ում',
  'intro.title': 'Ես {brand}-ն եմ՝ Ձեր իրավական և հարկային աջակիցը բարդ որոշումներում։',
  'intro.start': 'Հարցերի օրինակներ',
  'cites.label': 'Կարդացված հոդվածներ',
  'masthead.sub':
    'Ես {brand}-ն եմ՝ Ձեր իրավական և հարկային աջակիցը բարդ որոշումներում։ Պատասխանում եմ ՀՀ օրենսդրության իրական տեքստի հիման վրա՝ հղելով կոնկրետ հոդվածներին։',
  'card.expand': 'Ցույց տալ հոդվածը',
  'card.collapse': 'Ծալել',
  'oneshot.placeholder': 'Հարց ռուսերեն, հայերեն կամ լատինատառ (xanut bacel)',
  'oneshot.run': 'Կատարել',
  'oneshot.nothing': 'Ոչինչ չի գտնվել։ Կորպուսը՝ հարկային և աշխատանքային օրենսդրություն՝ Հարկային օրենսգիրք, Աշխատանքային օրենսգիրք, Կառավարության որոշումներ և ՊԵԿ հրամաններ։ Քաղաքացիական իրավունքը և դատական պրակտիկան դրա մեջ չեն մտնում։',
  'search.note': 'Մոդելը չի մասնակցում — միայն գտնված հատվածները',
  'search.found': 'Գտնվել է հատված՝',
  'example.1': 'Որքա՞ն է ԱԱՀ-ի դրույքաչափը։',
  'example.2': 'Ինչպե՞ս է հաշվարկվում արձակուրդային փոխհատուցումը աշխատանքից ազատվելիս։',
};

const ru: Dict = {
  'nav.consultations': 'Консультации',
  'nav.newCase': '+ Новая консультация',
  'nav.noCases': 'Пока нет сохранённых диалогов.',
  'nav.search': 'Поиск по диалогам',
  'nav.noMatches': 'Совпадений не найдено.',
  'nav.more': 'Действия',
  'nav.pin': 'Закрепить',
  'nav.unpin': 'Открепить',
  'nav.pinned': 'Закреплённые',
  'nav.showAllPinned': 'Показать все закреплённые',
  'nav.showLess': 'Свернуть',
  'nav.recent': 'Недавние',
  'nav.openRail': 'Открыть боковую панель',
  'nav.rename': 'Переименовать',
  'nav.renameTitle': 'Переименовать диалог',
  'nav.renameHint': 'Дайте название, чтобы легко находить этот диалог. Оставьте пустым — вернётся первый вопрос.',
  'nav.delete': 'Удалить',
  'nav.deleteAsk': 'Удалить навсегда?',
  'nav.deleteYes': 'Удалить',
  'nav.deleteTitle': 'Удалить диалог?',
  'nav.deleteBody': 'Диалог будет удалён навсегда, и у вас больше не будет к нему доступа. Это действие нельзя отменить.',
  'nav.cancel': 'Отмена',
  'nav.save': 'Сохранить',
  'account.profile': 'Профиль',
  'account.profileTitle': 'Изменить имя',
  'account.profileHint': 'Под этим именем вас видят участники вашего рабочего пространства.',
  'account.workspace': 'Рабочее пространство',
  'account.upgrade': 'Расширить тариф',
  'account.upgradeSoon': 'Оплата пока не подключена.',
  'account.questions': 'Вопросов на этой неделе',
  'ws.members': 'Участники',
  'ws.invitees': 'Приглашённые',
  'ws.usage': 'Использование',
  'ws.newMember': 'Новый участник',
  'ws.send': 'Пригласить',
  'ws.remove': 'Удалить',
  'ws.removeYes': 'Подтвердить',
  'ws.removeMemberTitle': 'Удалить участника?',
  'ws.removeMemberBody': 'Этот человек будет удалён из рабочего пространства и больше не будет расходовать недельные вопросы команды. Его аккаунт и диалоги сохранятся.',
  'ws.removeOwnerNote': 'Это создатель рабочего пространства. После удаления владельцем станете вы.',
  'ws.removeInviteTitle': 'Удалить приглашение?',
  'ws.removeInviteBody': 'Ссылка из приглашения перестанет работать. Пригласить снова можно в любой момент.',
  'ws.promote': 'Сделать админом',
  'ws.demote': 'Снять админа',
  'ws.roleLabel': 'Права',
  'ws.nameRequired': 'Укажите имя — приглашённый не сможет его ввести сам.',
  'ws.roleRequired': 'Выберите: админ или пользователь.',
  'ws.roleAdmin': 'Админ',
  'ws.roleMember': 'Пользователь',
  'ws.pending': 'Ожидает',
  'ws.you': 'вы',
  'ws.alreadyHere': 'Этот адрес уже в вашей команде или приглашён.',
  'ws.full': 'Команда заполнена.',
  'ws.failed': 'Не получилось. Попробуйте ещё раз.',
  'ws.totalThisMonth': 'Всего за эту неделю',
  'ws.inviteNote': 'Приглашение отправляется на указанный адрес. Оно сработает только при регистрации на этот же адрес.',

  'mode.chat': 'Диалог',
  'mode.ask': 'Разовый',
  'mode.search': 'Поиск',
  'mode.chatHint': 'Диалог с памятью — уточняйте и просите вывод по совокупности.',
  'mode.askHint': 'Один вопрос — один ответ, без памяти.',
  'mode.searchHint': 'Только найденные фрагменты, без модели.',

  'corpus.acts': 'актов',
  'corpus.chunks': 'фрагментов',
  'corpus.synced': 'Сверено с ARLIS',
  'corpus.disclaimer': 'Не юридическая консультация — проверяйте полный текст',

  'turn.question': 'Вопрос',
  'turn.searchedFor': 'искали:',

  'stage.understanding': 'Разбираю вопрос…',
  'stage.searching': 'Ищу в Налоговом кодексе…',
  'stage.reading': 'Читаю найденные статьи…',
  'stage.writing': 'Формулирую ответ…',

  'coverage.partial':
    'Найденные статьи покрывают вопрос частично — проверьте, что уточняющий вопрос ниже не меняет вывод.',
  'coverage.none':
    'Прямой нормы по этому вопросу в найденных статьях нет. Ниже — только смежные положения.',


  'composer.first': 'Опишите ситуацию или задайте вопрос…',
  'composer.next': 'Уточняющий вопрос или «сделай вывод по всему обсуждённому»',
  'composer.send': 'Отправить',

  'norm.empty':
    'Задайте вопрос — здесь появится статья, на которую опирается ответ, с выделенной цитатой и датой редакции.',
  'norm.inForce': 'Действует',
  'norm.revised': 'Ред.',
  'norm.adopted': 'Принят',
  'norm.revisedFrom': 'Ред. от',
  'norm.checked': 'Сверено',
  'norm.copyQuote': 'Скопировать цитату',
  'norm.copyArticle': 'Скопировать статью',
  'norm.copied': 'Скопировано',
  'norm.refersTo': 'Ссылается на',

  'login.note':
    'Доступ по общему паролю. Каждый ответ расходует платные API-запросы, поэтому инструмент закрыт.',
  'login.password': 'Пароль',
  'login.enter': 'Войти',
  'login.wrong': 'Неверный пароль.',
  'auth.signIn': 'Вход',
  'auth.register': 'Регистрация',
  'auth.haveAccount': 'Уже есть аккаунт?',
  'auth.noAccount': 'Ещё нет аккаунта?',
  'auth.signInNote': 'Войдите, чтобы увидеть свои консультации. Каждый видит только свои.',
  'auth.registerNote': 'Создайте аккаунт. Бесплатный тариф включает 5 вопросов в неделю.',
  'auth.email': 'Эл. почта',
  'auth.name': 'Имя',
  'auth.fullName': 'Имя и фамилия',
  'auth.companyName': 'Название компании',
  'auth.companySize': 'Размер компании',
  'auth.next': 'Продолжить',
  'auth.inviteOffer':
    'Пригласите коллег и получите +{n} вопросов. За каждого зарегистрировавшегося — ещё +{m}.',
  'auth.inviteNote':
    'По желанию. +{m} начисляются в момент регистрации коллеги. Адреса третьим лицам не передаём.',
  'auth.skip': 'Пропустить',
  'auth.inviteAnother': 'Добавить ещё',
  'auth.whyWeAsk':
    'Эти данные нужны только чтобы предложить вам подходящий тариф и адаптировать ответы под вашу работу. Третьим лицам не передаём.',
  'auth.createAccount': 'Создать аккаунт',
  'auth.or': 'или',
  'reset.forgot': 'Забыли пароль?',
  'reset.send': 'Отправить ссылку для восстановления',
  'reset.maybeSent': 'Если аккаунт с этим адресом существует, ссылка отправлена. Действует 1 час.',
  'reset.chooseNew': 'Выберите новый пароль.',
  'reset.save': 'Сохранить пароль',
  'reset.invalid': 'Ссылка недействительна.',
  'reset.expired': 'Срок ссылки истёк. Запросите новую.',
  'reset.used': 'Ссылка уже использована. Запросите новую.',
  'invite.loading': 'Открываем приглашение…',
  'invite.gone': 'Приглашение недействительно или уже принято.',
  'invite.invitedYou': 'пригласил вас. Осталось выбрать пароль.',
  'invite.accept': 'Принять приглашение',
  'invite.failed': 'Не удалось. Попробуйте ещё раз.',
  'invite.thenMember': 'После подтверждения вы появитесь в списке участников команды.',
  'auth.oauth.noAccount': 'Учётная запись не найдена. Зарегистрируйтесь.',
  'auth.oauth.cancelled': 'Вход отменён.',
  'auth.oauth.failed': 'Не удалось войти через Google. Попробуйте ещё раз.',
  'auth.oauth.unverified': 'Адрес Google не подтверждён.',
  'auth.google': 'Продолжить с Google',
  'auth.badCredentials': 'Неверная почта или пароль.',
  'auth.verify.title': 'Проверьте почту',
  'auth.verify.body': 'Ссылка для подтверждения отправлена на адрес:',
  'auth.verify.hint': 'Ссылка действует 24 часа. Не видно письма — проверьте папку «Спам».',
  'auth.verify.resend': 'Отправить ещё раз',
  'auth.verify.resent': 'Отправлено',
  'auth.verify.unverified': 'Адрес ещё не подтверждён. Проверьте почту или отправьте ссылку заново.',
  'auth.verify.sendFailed': 'Письмо отправить не удалось. Попробуйте отправить ещё раз.',
  'auth.verify.working': 'Подтверждаем…',
  'auth.verify.invalid': 'Ссылка недействительна.',
  'auth.verify.expired': 'Срок ссылки истёк. Отправьте новую.',
  'auth.verify.alreadyUsed': 'Адрес уже подтверждён. Можно входить.',
  'auth.verify.toSignIn': 'Ко входу',
  'auth.emailTaken': 'Аккаунт с этой почтой уже существует.',
  'auth.weakPassword': 'Пароль должен быть не короче 8 символов.',
  'auth.invalidEmail': 'Неверный адрес почты.',
  'auth.confirmPassword': 'Повторите пароль',
  'auth.passwordMismatch': 'Пароли не совпадают.',
  'auth.showPassword': 'Показать пароль',
  'auth.hidePassword': 'Скрыть пароль',
  'auth.signOut': 'Выйти',
  'auth.quotaOut': 'Лимит вопросов на эту неделю исчерпан.',
  'share.share': 'Поделиться',
  'share.title': 'Поделиться диалогом',
  'share.copy': 'Копировать',
  'share.failed': 'Не удалось получить ссылку. Попробуйте ещё раз.',
  'share.publicNote': 'Ссылка публичная: любой, у кого она есть, сможет прочитать этот диалог без входа в аккаунт.',
  'share.copied': 'Ссылка скопирована',
  'share.stop': 'Закрыть доступ',
  'share.shared': 'Открыт доступ',
  'share.gone': 'Эта ссылка больше не действует.',
  'share.readOnly': 'Консультация по ссылке',
  'share.ownQuestion': 'Есть свой вопрос?',
  'share.openTool': 'Открыть MatyanAI',
  'preview.lede': 'Задайте свой правовой вопрос.',
  'preview.ask': 'Спросить',
  'preview.thinking': 'Ищу…',
  'preview.rest': 'Остальная часть ответа — со статьями и дословными цитатами',
  'preview.sources': 'статей',
  'preview.unlock': 'Зарегистрироваться и увидеть всё',
  'preview.free': 'Бесплатно · 5 вопросов в неделю',
  'preview.failed': 'Не удалось получить ответ.',
  'preview.resuming': 'Готовлю ваш полный ответ…',

  'theme.auto': 'Авто',
  'theme.light': 'Светлая',
  'theme.dark': 'Тёмная',

  'error.credit':
    'Закончился баланс Anthropic API — это не ошибка приложения. Пополните счёт в Plans & Billing, поиск работает без него.',
  'error.noApi': 'API не отвечает',
  'norm.title': 'Источники',
  'norm.carried': 'Перенесено из прошлых сообщений — прочитано, но в этом ответе не процитировано.',
  'norm.openArlis': 'Открыть в ARLIS',
  'intro.title': 'Я {brand} — ваша правовая и налоговая поддержка в сложных решениях.',
  'intro.start': 'Примеры вопросов',
  'cites.label': 'Прочитанные статьи',
  'masthead.sub':
    'Я {brand} — ваша правовая и налоговая поддержка в сложных решениях. Отвечаю по реальному тексту законодательства РА, со ссылками на конкретные статьи.',
  'card.expand': 'Показать статью',
  'card.collapse': 'Свернуть',
  'oneshot.placeholder': 'Вопрос на русском, армянском или латиницей (xanut bacel)',
  'oneshot.run': 'Выполнить',
  'oneshot.nothing': 'Ничего не найдено. Корпус — налоговое и трудовое законодательство: Налоговый кодекс, Трудовой кодекс, решения Правительства и приказы КГД. Гражданское право и судебная практика в него не входят.',
  'search.note': 'Модель не участвует — только найденные фрагменты',
  'search.found': 'Найдено фрагментов:',
  'example.1': 'Какая ставка НДС в Армении?',
  'example.2': 'Отпускные при увольнении в середине месяца',
};

const en: Dict = {
  'nav.consultations': 'Consultations',
  'nav.newCase': '+ New consultation',
  'nav.noCases': 'No saved consultations yet.',
  'nav.search': 'Search conversations',
  'nav.noMatches': 'No matches found.',
  'nav.more': 'Actions',
  'nav.pin': 'Pin',
  'nav.unpin': 'Unpin',
  'nav.pinned': 'Pinned',
  'nav.showAllPinned': 'Show all pinned',
  'nav.showLess': 'Show less',
  'nav.recent': 'Recent',
  'nav.openRail': 'Open sidebar',
  'nav.rename': 'Rename',
  'nav.renameTitle': 'Rename conversation',
  'nav.renameHint': 'Give it a name so you can find it easily. Leave it empty to go back to the first question.',
  'nav.delete': 'Delete',
  'nav.deleteAsk': 'Delete permanently?',
  'nav.deleteYes': 'Delete',
  'nav.deleteTitle': 'Delete conversation?',
  'nav.deleteBody': 'This conversation will be deleted permanently and you will no longer have access to it. This cannot be undone.',
  'nav.cancel': 'Cancel',
  'nav.save': 'Save',
  'account.profile': 'Profile',
  'account.profileTitle': 'Change your name',
  'account.profileHint': 'This is the name members of your workspace see.',
  'account.workspace': 'Workspace',
  'account.upgrade': 'Upgrade plan',
  'account.upgradeSoon': 'Payments are not switched on yet.',
  'account.questions': 'Questions this week',
  'ws.members': 'Members',
  'ws.invitees': 'Invitees',
  'ws.usage': 'Usage',
  'ws.newMember': 'New member',
  'ws.send': 'Invite',
  'ws.remove': 'Remove',
  'ws.removeYes': 'Confirm',
  'ws.removeMemberTitle': 'Remove member?',
  'ws.removeMemberBody': 'They will be removed from your workspace and will stop using the team\'s weekly questions. Their account and conversations are kept.',
  'ws.removeOwnerNote': 'This is the person who created the workspace. Once removed, you become its owner.',
  'ws.removeInviteTitle': 'Remove invitation?',
  'ws.removeInviteBody': 'The invitation link will stop working. You can invite them again at any time.',
  'ws.promote': 'Make admin',
  'ws.demote': 'Remove admin',
  'ws.roleLabel': 'Rights',
  'ws.nameRequired': 'Give a name — the person you invite cannot fill it in themselves.',
  'ws.roleRequired': 'Choose one: admin or member.',
  'ws.roleAdmin': 'Admin',
  'ws.roleMember': 'User',
  'ws.pending': 'Pending',
  'ws.you': 'you',
  'ws.alreadyHere': 'That address is already in your team or invited.',
  'ws.full': 'The team is full.',
  'ws.failed': 'That did not work. Try again.',
  'ws.totalThisMonth': 'Total this week',
  'ws.inviteNote': 'An invitation is emailed to that address. It only applies if they register with the same address.',

  'mode.chat': 'Dialogue',
  'mode.ask': 'One-shot',
  'mode.search': 'Search',
  'mode.chatHint': 'Dialogue with memory — refine, then ask for a conclusion across everything.',
  'mode.askHint': 'One question, one answer, no memory.',
  'mode.searchHint': 'Retrieved fragments only, no model involved.',

  'corpus.acts': 'acts',
  'corpus.chunks': 'fragments',
  'corpus.synced': 'Checked against ARLIS',
  'corpus.disclaimer': 'Not legal advice — verify the full text',

  'turn.question': 'Question',
  'turn.searchedFor': 'searched for:',

  'stage.understanding': 'Reading the question…',
  'stage.searching': 'Searching the Tax Code…',
  'stage.reading': 'Reading the articles found…',
  'stage.writing': 'Drafting the answer…',

  'coverage.partial':
    'The articles found cover this only partly — check whether the clarifying question below changes the conclusion.',
  'coverage.none':
    'No provision answering this was found. Below are related provisions only.',


  'composer.first': 'Describe the situation or ask a question…',
  'composer.next': 'A follow-up, or “summarise everything discussed”',
  'composer.send': 'Send',

  'norm.empty':
    'Ask a question — the article the answer rests on appears here, with the quoted passage marked and the revision date shown.',
  'norm.inForce': 'In force',
  'norm.revised': 'Rev.',
  'norm.adopted': 'Adopted',
  'norm.revisedFrom': 'Revised',
  'norm.checked': 'Checked',
  'norm.copyQuote': 'Copy quotation',
  'norm.copyArticle': 'Copy article',
  'norm.copied': 'Copied',
  'norm.refersTo': 'Refers to',

  'login.note':
    'Shared-password access. Every answer spends paid API requests, so the tool is closed.',
  'login.password': 'Password',
  'login.enter': 'Sign in',
  'login.wrong': 'Wrong password.',
  'auth.signIn': 'Sign in',
  'auth.register': 'Register',
  'auth.haveAccount': 'Already have an account?',
  'auth.noAccount': 'No account yet?',
  'auth.signInNote': 'Sign in to see your consultations. Each account sees only its own.',
  'auth.registerNote': 'Create an account. The free plan includes 5 questions a week.',
  'auth.email': 'Email',
  'auth.name': 'Name',
  'auth.fullName': 'Full name',
  'auth.companyName': 'Company name',
  'auth.companySize': 'Company size',
  'auth.next': 'Continue',
  'auth.inviteOffer':
    'Invite colleagues and get +{n} questions. Another +{m} for each one who registers.',
  'auth.inviteNote':
    'Optional. The +{m} arrives when your colleague registers. Addresses are never shared with anyone else.',
  'auth.skip': 'Skip',
  'auth.inviteAnother': 'Add another',
  'auth.whyWeAsk':
    'We use this only to suggest the right plan and keep answers relevant to your work. Never shared with anyone else.',
  'auth.createAccount': 'Create account',
  'auth.or': 'or',
  'reset.forgot': 'Forgot your password?',
  'reset.send': 'Send a reset link',
  'reset.maybeSent': 'If an account exists for that address, a link has been sent. It is valid for 1 hour.',
  'reset.chooseNew': 'Choose a new password.',
  'reset.save': 'Save password',
  'reset.invalid': 'This link is not valid.',
  'reset.expired': 'This link has expired. Request a new one.',
  'reset.used': 'This link has already been used. Request a new one.',
  'invite.loading': 'Opening the invitation…',
  'invite.gone': 'This invitation is not valid, or has already been accepted.',
  'invite.invitedYou': 'invited you. All that is left is choosing a password.',
  'invite.accept': 'Accept the invitation',
  'invite.failed': 'That did not work. Please try again.',
  'invite.thenMember': 'Once confirmed, you will appear in the team members list.',
  'auth.oauth.noAccount': 'No account found. Please register.',
  'auth.oauth.cancelled': 'Sign-in cancelled.',
  'auth.oauth.failed': 'Google sign-in failed. Please try again.',
  'auth.oauth.unverified': 'That Google address is not verified.',
  'auth.google': 'Continue with Google',
  'auth.badCredentials': 'Wrong email or password.',
  'auth.verify.title': 'Check your email',
  'auth.verify.body': 'A confirmation link was sent to:',
  'auth.verify.hint': 'The link is valid for 24 hours. No message? Check your spam folder.',
  'auth.verify.resend': 'Send again',
  'auth.verify.resent': 'Sent',
  'auth.verify.unverified': 'This address is not confirmed yet. Check your email or send the link again.',
  'auth.verify.sendFailed': 'The message could not be sent. Try sending it again.',
  'auth.verify.working': 'Confirming…',
  'auth.verify.invalid': 'This link is not valid.',
  'auth.verify.expired': 'This link has expired. Send a new one.',
  'auth.verify.alreadyUsed': 'This address is already confirmed. You can sign in.',
  'auth.verify.toSignIn': 'Go to sign in',
  'auth.emailTaken': 'An account with this email already exists.',
  'auth.weakPassword': 'Password must be at least 8 characters.',
  'auth.invalidEmail': 'Invalid email address.',
  'auth.confirmPassword': 'Repeat password',
  'auth.passwordMismatch': 'Passwords do not match.',
  'auth.showPassword': 'Show password',
  'auth.hidePassword': 'Hide password',
  'auth.signOut': 'Sign out',
  'auth.quotaOut': "This week's question limit is used up.",
  'share.share': 'Share',
  'share.title': 'Share conversation',
  'share.copy': 'Copy',
  'share.failed': 'Could not get the link. Please try again.',
  'share.publicNote': 'This link is public: anyone who has it can read this conversation, no account needed.',
  'share.copied': 'Link copied',
  'share.stop': 'Stop sharing',
  'share.shared': 'Shared',
  'share.gone': 'This link is no longer active.',
  'share.readOnly': 'Shared consultation',
  'share.ownQuestion': 'Have a question of your own?',
  'share.openTool': 'Open MatyanAI',
  'preview.lede': 'Ask your legal question.',
  'preview.ask': 'Ask',
  'preview.thinking': 'Searching…',
  'preview.rest': 'The rest of the answer — with the articles and verbatim quotes',
  'preview.sources': 'articles',
  'preview.unlock': 'Register and see it all',
  'preview.free': 'Free · 5 questions a week',
  'preview.failed': 'Could not get an answer.',
  'preview.resuming': 'Preparing your full answer…',

  'theme.auto': 'Auto',
  'theme.light': 'Light',
  'theme.dark': 'Dark',

  'error.credit':
    'The Anthropic API balance is exhausted — this is not an application fault. Top up in Plans & Billing; search works without it.',
  'error.noApi': 'The API is not responding',
  'norm.title': 'Sources',
  'norm.carried': 'Carried from earlier messages — read, but not quoted in this answer.',
  'norm.openArlis': 'Open in ARLIS',
  'intro.title': 'I am {brand} — your legal and tax support in difficult decisions.',
  'intro.start': 'Example questions',
  'cites.label': 'Articles read',
  'masthead.sub':
    'I am {brand} — your legal and tax support in difficult decisions. I answer from the real text of Armenian law, citing the articles themselves.',
  'card.expand': 'Show the article',
  'card.collapse': 'Collapse',
  'oneshot.placeholder': 'A question in Russian, Armenian or Latin script (xanut bacel)',
  'oneshot.run': 'Run',
  'oneshot.nothing': 'Nothing found. The corpus is tax and labour law: the Tax Code, the Labour Code, government decisions and SRC orders. Civil law and court practice are not in it.',
  'search.note': 'No model involved — retrieved fragments only',
  'search.found': 'Fragments found:',
  'example.1': 'What is the VAT rate in Armenia?',
  'example.2': 'How is unused leave compensated on dismissal?',
};

const DICTS: Record<Lang, Dict> = { hy, ru, en };

const STORAGE_KEY = 'armlex.lang';

/** Remembered choice, else the browser's preference, else Russian. */
export function initialLang(): Lang {
  const saved = localStorage.getItem(STORAGE_KEY);
  if (saved === 'hy' || saved === 'ru' || saved === 'en') return saved;
  const nav = navigator.language.slice(0, 2).toLowerCase();
  if (nav === 'hy') return 'hy';
  if (nav === 'en') return 'en';
  return 'ru';
}

export function storeLang(lang: Lang): void {
  localStorage.setItem(STORAGE_KEY, lang);
  // Drives font selection and screen-reader pronunciation for the chrome.
  // Armenian legal text carries its own lang="hy" regardless.
  document.documentElement.lang = lang;
}

/**
 * Look up a string. Falls back to Russian, then to the key itself — a visible
 * key is a bug report, which beats an empty element that hides the omission.
 */
export function translator(lang: Lang) {
  return (key: string): string => DICTS[lang][key] ?? ru[key] ?? key;
}
