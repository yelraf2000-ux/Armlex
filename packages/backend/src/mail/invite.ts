...
export async function sendInvite(opts: {
  ...
  const html = [
    ...
    `<p style="margin:0 0 24px"><a href="${link}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:8px;font-size:15px;font-weight:600">${escapeHtml(c.button)}</a></p>`,
    `<p style="margin:0 0 4px;font-size:13px;color:#334155"><strong>${escapeHtml(c.useThis)}</strong></p>`,
    `<p style="margin:0 0 16px;font-size:13px;color:#64748b">${escapeHtml(c.free)}</p>`,
    `<p style="margin:0;font-size:13px;color:#64748b">${escapeHtml(c.ignore)}</p>`,
    '</div></div>',
  ].join('');
