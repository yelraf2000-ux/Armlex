...
- **2026-09-09 → 09-12 — Workspace, routing and the account menu.** Every
  screen has an address (`/c/<id>`, `/workspace/members`, `/workspace/usage`,
  `/profile`, `/login`, `/registration`). Inviting requires a name and a rank
  (default Օգտվող). Free allowance: 10 for the account creator, +5 per
  colleague. Registration's invite step has one Continue button. Popups for
  rename and profile name. Sidebar collapses to a thin strip. Everything
  Russian and English removed — interface, emails and answers are Armenian
  only (`i18n.ts` is a single dictionary; answers forced `ANSWER LANGUAGE:
  ARMENIAN`). Conversation title generated by Haiku BEFORE the answer.

- **2026-09-13 → 09-14 — Sessions that end.** Cookie is now
  `<userId>.<version>.<expiresAtMs>.<hmac>`: 7-day IDLE window, refreshed at
  half-life; `users.session_version` (migration 017) makes sign-out revoke every
  copy. Opening a conversation starts at its top, not its foot.

- **2026-09-15 → 09-16 — The sources column, rebuilt around what the answer
  used.** Shows only provisions the answer names or quotes (`cited.ts`), one
  entry per act, act title on one line. Each provision shows its own title and
  its operative passage; expanding opens ONLY the part(s) the answer cites
  (`parts.ts`: named part wins; a quote must occur in exactly one part to
  locate; no anchor → whole article). Measured on 263 answers: 63% name a part,
  7.6% offer no anchor. Column fills as the answer cites and never shrinks
  (turn is `streaming` from creation; unsettled text gets no fallback, and the
  end of growing text is not a right boundary). Nothing shows before the first
  citation. Seven-second wait shows verbatim statute from the Tax/Labour Codes
  (`answer/interlude.ts`, `GET /api/interlude`, public).

- **2026-09-16 — Design system adopted.** `docs/DESIGN.md` is the spec. Noto
  Sans + Noto Sans Armenian, blue #2563eb palette, 8pt spacing, radius scale,
  shadows; component layer appended to `styles.css`. Light only — dark theme,
  `theme.ts` and the hidden switcher removed. Emails repainted. Fixed on the
  way: destructive actions were blue; the open conversation was never
  highlighted; a hover specificity bug; the account-menu upgrade link was never
  blue; statute had been rendering in CAPITALS via an `.act` class collision.
  «Ընդլայնել սակագինը» is a primary button in the menu and on the usage page.

- **2026-09-16 → 09-17 — Logo.** Book mark (`public/logo.png`) beside the name
  in every header; full lockup (`public/logo-lockup.png`) replaces it while the
  sidebar is open; nothing in the header while collapsed on wide screens (the
  strip carries the book). Favicons `favicon-32.png`, `apple-touch-icon.png`;
